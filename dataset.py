...
class ForafricDataset(utils.Dataset):
    def load_dataset(self, dataset_dir, load_small=False, return_coco=True):
        ...
        # Load all classes (Only field in this version)
        ...
        for _class_id in classIds:
            self.add_class("coco", _class_id, self.coco.loadCats(_class_id)[0]["name"])

        # Register Images
        for _img_id in image_ids:
            assert(os.path.exists(os.path.join(image_dir, self.coco.imgs[_img_id]['file_name'])))
            self.add_image(
                "coco", image_id=_img_id,
    ...
    def load_mask(self, image_id):
        ...
        if class_ids:
            ...
            return super(ForafricDataset, self).load_mask(image_id)
